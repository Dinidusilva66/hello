...
async def pm_next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("◀️ ආපසු", callback_data=f"pmnext_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"📖 පිටු {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages")]                                  
        )
    elif off_set is None:
        btn.append(
            [InlineKeyboardButton(f"📖 {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages"),
             InlineKeyboardButton("ඊලග ▶️", callback_data=f"pmnext_{req}_{key}_{n_offset}")])
    else:
        btn.append([
            InlineKeyboardButton("◀️ ආපසු", callback_data=f"pmnext_{req}_{key}_{off_set}"),
            InlineKeyboardButton(f"📖 {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages"),
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if offset != "":
        ...
        btn.append(
            ...
            InlineKeyboardButton(text="ඊලග ▶️", callback_data=f"pmnext_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
